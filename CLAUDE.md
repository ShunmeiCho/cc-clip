...
Cursor notifications go through `~/.cursor/hooks.json` (`internal/shim/cursor.go`), merged rather than dropped because the file may already hold the user's own hooks. Two things differ from every other adapter. The hook command addresses the binary **absolutely** (`sh -c 'exec "$HOME/.local/bin/cc-clip" ...'`) rather than as a bare `cc-clip`: Cursor runs a user hook from `~/.cursor` with the system environment, not a login shell, and the reported failure mode when `~/.local/bin` is off PATH is a hook that exits 127 while Cursor reports nothing — the same remote-PATH hazard recorded above. And `runCursorNotify` writes **nothing** to stdout: Cursor reads a stop hook's stdout as an optional `{"followup_message": ...}` and restarts the agent when it finds one, so an adapter that printed anything would turn every finished turn into a new one.
