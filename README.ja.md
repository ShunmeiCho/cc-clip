...
<p align="center">
  <b>Claude Code、Codex CLI、opencode で SSH 越しに画像を貼り付け、Claude Code、Codex CLI、opencode にデスクトップ通知を届けます。</b>
</p>
<p align="center">
...
ひとつのツールで済みます。Claude Code、Codex、opencode の変更は不要です。3 つすべてでクリップボードが使え、通知は Claude Code、Codex CLI、opencode 向けに配線されています。
...
**リモート:** Linux。SSH アクセス、`curl`、`bash`、そして少なくとも 1 つのクリップボード backend が必要です。X11 consumer には `xclip`、Wayland consumer には `wl-paste` を使います。macOS の tunnel/shim 経路は `cc-clip connect` が自動設定します。Windows の upload/hotkey 経路は SSH/SCP を直接使います。
