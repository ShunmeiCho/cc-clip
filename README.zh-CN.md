...
> **经验规则：** `--codex` **只**安装 Codex 支持（Xvfb + x11-bridge，不装 Claude shim）。仅在此 host 运行 Codex CLI 而不用 Claude Code 时用它；同时运行**两者**时用 `--all`；普通 `setup`（或 `--claude`）覆盖 Claude Code 和 opencode。
...
只需要记住这三种情况。右列适用于同时运行 **Claude Code 和 Codex CLI** 的 host（用 `--all`）；若只运行 Codex CLI，则用 `--codex` 代替。左列（普通命令）覆盖 Claude Code 和 opencode。

| 场景 | 命令（只用 Claude Code） | 命令（Claude Code + Codex CLI） |
|---|---|---|
...
| `cc-clip setup <host> --all` | 面向 Claude Code + Codex 的完整初始化（`--codex` 单独 = 仅 Codex） |
