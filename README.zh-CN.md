...
| 目标 | 命令 | 会增加什么 |
|---|---|---|
| Claude Code（默认） | `cc-clip setup myserver`（或 `--claude`） | Claude shim |
| 只用 Codex CLI | `cc-clip setup myserver --codex` | 仅 Codex（Xvfb + x11-bridge）—— **不装** Claude shim |
| Claude + Codex | `cc-clip setup myserver --all` | 两者都装 |
| opencode | `cc-clip setup myserver --opencode` | 剪贴板 shim + opencode-notify（转发 `session.idle`） |
| Antigravity (agy) | `cc-clip setup myserver --agy` | agy 通知插件（目前仅通知） |

> **相对 v0.8.1 的破坏性变更：** 在 beta 版中，`--codex` **只**安装 Codex 支持（不装 Claude shim），而在稳定版 v0.8.1 中 `--codex` 是在 Claude shim 之上**追加** Codex。Codex 目标（`--codex` 或 `--all`）需要远程已安装 Xvfb（有 passwordless `sudo` 时会自动安装，否则请手动安装）。
...
| Claude Code | ⚠️ 手动 — 将 `cc-clip-hook` 添加到 `~/.claude/settings.json` |
| opencode | ❌ 尚未开箱支持 |
...
| CLI | 图片粘贴 | 通知 |
|-----|-------------|----------------|
| [Claude Code](https://www.anthropic.com/claude-code) | ✅ 开箱可用（xclip / wl-paste shim） | ✅ 通过 `Stop` / `Notification` hooks 中的 `cc-clip-hook`（手动配置） |
...
**远程：** Linux，具备 SSH 访问、`curl`、`bash`，以及至少一种剪贴板 backend：X11 consumer 使用 `xclip`，Wayland consumer 使用 `wl-paste`。本地 macOS tunnel/shim 路径由 `cc-clip connect` 自动配置；Windows 上传/hotkey 路径直接使用 SSH/SCP。
